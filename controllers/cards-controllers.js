...
const getSingleTagByNameQuery=require('../models/tag')
const createTagQuery=require('../models/tag')

const createCard = async (tagName) => {
  const existingTag = await getSingleTagByNameQuery(tagName)
  let createdTag
  if (!existingTag.rows.length) {
    createdTag= await createTagQuery(tagName)
  }
